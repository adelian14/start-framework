...
export default function Navbar() {
  ...
  return (
    <nav className='bg-slate-700 text-white md:flex justify-between items-center lg:px-44 md:px-14 px-6 py-9 relative'>
      <div>
        <h1 className='lg:text-3xl text-xl font-bold uppercase'><Link to={'/'}>Start Framework</Link></h1>
      </div>
      <div>
        <ul className={`${navVisivle?'flex':'hidden'} md:flex justify-between md:gap-10 gap-4 md:flex-row flex-col mt-5 md:mt-0`}>
          <li className='lg:text-lg uppercase font-medium'><NavLink to={'/about'}>About</NavLink></li>
          <li className='lg:text-lg uppercase font-medium'><NavLink to={'/portfolio'}>Portfolio</NavLink></li>
          <li className='lg:text-lg uppercase font-medium'><NavLink to={'/contact'}>Contact</NavLink></li>
        </ul>
      </div>
